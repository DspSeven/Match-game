...
class GamePage extends Component {
  ...
  selectedCategory = tabId => {
    this.setState({categoryType: tabId})
  }

  // display selected image
  displaySelectedImage = displayImage => {
    console.log(displayImage)
    /*
    const {score} = this.state
    const randomNumber = Math.floor(Math.random() * 30)
    const imageUniformResourceLocator = imagesList[randomNumber].imageUrl
    this.setState({image: imageUniformResourceLocator, score: score + 1})
    */
  ...
  render() {
    const {categoryType, timer} = this.state
    // filtering list based on category
    const searchResults = imagesList.filter(eachCategory =>
      eachCategory.category.includes(categoryType),
    )
    return (
      <div>
        {this.gameBar()}
        {timer === 0 ? this.finishGame() : this.getGamePage(searchResults)}
      </div>
    )
  }
}
